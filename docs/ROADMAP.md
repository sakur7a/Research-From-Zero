...
| #22 | Previous connector snapshot remains 1 complete, 1 partial, 3 pending human review and 1 unknown. `scripts/agent_real_http_browser_smoke.py` passes both local HTTP (10 stages) and hosted test mode over self-signed loopback HTTPS (14 stages), including two isolated guest cookies, refused cross-guest task access/cancel/export, matrix, source read-back, reused-evidence follow-up, approval and export. The dedicated CI job repeats the browser suites and publishes their reports in the workflow summary. | Check the current workflow run for CI status. Human review of pending sample expectations, public TLS/selected-host acceptance and real BYOK remain outstanding; the owner marked host/model/cost authorization `未定`. |
...
Current local verification counts are recorded in `docs/TESTING.md`; Chromium and MockTransport checks use fixture providers and do not close hosted release gates. Container and install smoke passed in CI run 35911961690. Docker is not installed on the local Windows workstation; the browser smoke CI result must be taken from the current workflow run.
