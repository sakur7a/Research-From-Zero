...
| #20 | Implemented locally: one task-wide upstream HTTP counter/deadline/cancel boundary now reaches model and tool requests; stops between scholarly sources preserve completed-source evidence. The composer exposes focused/expanded initial scope, exact serialized request chars/bytes and per-provider usage; repeated tool inputs are called out while provider cache/explicit refresh remain usable. One owner-scoped progress endpoint feeds incremental event/evidence pages; polling is single-request, hidden-tab slowed, and respects 401/429/offline backoff. Three-run MockTransport A/B on the same long-abstract/large-resource fixture: model/tool/upstream calls and report correctness stayed at 7/7/24 with 29 evidence and 3 resource links; serialized bodies fell from 453,427 bytes / 425,687 chars (largest 104,323 bytes) at `cbb4a82` to 340,473 bytes / 312,127 chars (largest 79,839 bytes), about 24.9% fewer total bytes. Median harness time was 1.10s baseline vs 1.62s current; this fixture shows no latency gain and is not a real-provider performance claim. | Human/real-model quality and provider billing still need the authorization recorded in #1; all current checks are fixtures. |
| #21 | Implemented locally: site emergency stop is deployer-controlled; destination breakers persist by normalized Base URL without keys; 429 cooldowns and model-list/connection-test attempt ledgers are per owner; probes are capped at 6 list requests/minute and 3 connection tests/5 minutes, one concurrent per owner and four across the service. Failed/successful calls only update their destination, and probes do not occupy the research slot. | Fixture coverage demonstrates Provider 1 outage isolation, Provider 2 success without clearing Provider 1, per-owner Retry-After cooldown, one half-open probe, ledger persistence and refusal before HTTP. Live-provider behavior remains untested; no paid endpoint was called. |
