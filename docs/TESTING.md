...
- Docker is unavailable on the local Windows workstation, so the image start/restart path has not been claimed as locally verified. GitHub container CI is configured but must pass on the pushed commit. No Render account/service/URL was created; the owner has not accepted temporary storage or hosting/bandwidth exposure, and HTTPS/user acceptance remains in #22/#23.
