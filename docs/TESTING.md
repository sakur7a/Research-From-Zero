...
- The complete fixed-fixture Python suite passed: **595 passed**, with one existing Starlette `BlockingPortal` deprecation warning. This run used MockTransport for model/provider boundaries; no live model endpoint, user key or paid request was used.
...
- `compileall` and `git diff --check` passed after the runtime deadline and fake-clock regression were added. The progress pagination fixture takes its cursor after the completed task's final event; the final full-suite rerun passed all 595 tests.
...
- Matrix schema 3 carries `scope` through each JSON row and into Markdown/CSV and both matrix views. A row with an empty scope stays empty; the renderer does not infer what a provider checked.
- On explicit confirmation, the appended library observation retains the selected run ID, paper/resource evidence IDs, association source URLs, locators and retrieval times, the model's proposal note, resource check time/scope and the user's confirmation time. The library evidence history shows this separately from the provider audit; that approval does not promote `attribution` or `version_match`. `/api/export` retains the same record.
- The task's matrix JSON, Markdown, CSV and confirmed library records now have regression checks for paper/version, resource URL, status, check time/scope, source coverage and association evidence. The web result lets the reader select multiple links, preview without writes, confirm into the owner-scoped library, and repeat safely without duplicating observations or replacing notes. A guessed evidence ID is refused.
- Verification: the full Python suite passed **596 tests** with one existing Starlette deprecation warning; `npm test` passed **84 tests** and `npm run check` passed. Agent Chromium passed **15 stages** and library Chromium passed **16 stages**, with zero page errors. These paths use fixtures/MockTransport and do not measure real-model quality or relationship accuracy.
