...
async function openEvidence(paper,resource) {
  ...
  function show(index) {
    ...
    const associationApprovals=(observation.agent_association_approvals||[]).map(approval=>{
      const sources=(approval.proposal_sources||[]).map(source=>`<li><code>${e(source.evidence_id)}</code> · ${e(source.locator||'来源定位未记录')} · ${e(timeLabel(source.retrieved_at))}${source.source_url?` · <a href="${link(source.source_url)}" target="_blank" rel="noopener noreferrer">打开来源 ↗</a>`:''}</li>`).join('');
      return `<article class="association-approval"><strong>${approval.decision==='confirmed_by_user'?'用户已批准保存此候选关联':'关联决策'}</strong><p>论文证据 <code>${e(approval.paper_evidence_id)}</code> · 资源证据 <code>${e(approval.resource_evidence_id)}</code></p><p>由模型提出，说明：${e(approval.proposal_note||'未提供')}</p><p>确认时间 ${e(timeLabel(approval.confirmed_at))} · 检查范围 ${e(approval.resource_scope||'未记录')}</p><ul>${sources}</ul></article>`;
    }).join('');
    ...
    ${associationApprovals?`<h3 class="evidence-section-title">Agent 候选关联保存记录</h3><div class="association-approvals">${associationApprovals}</div>`:''}
