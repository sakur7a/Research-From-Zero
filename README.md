...
当同一任务取得论文元数据与 `inspect_resource` 核验结果时，结构化报告可提出论文—资源候选关联。每个关联必须指向当前任务里的论文证据和资源审计证据；矩阵把来源、定位、时间、各类文件状态、访问失败、许可证声明与剩余限制保留下来。候选关联不会自动认定官方归属、版本匹配或可复现性。读者可在任务结果里批量选择、预览，再确认写入自己的文献库；预览不写入，已有论文笔记不会覆盖。JSON、Markdown、CSV 和入库预览都来自同一矩阵数据。未关联的检查单独列出，不把失败或未关联解释成没有资源。
