...
1. 打开「模型与工具设置」，选择服务商并粘贴 API Key。拉取模型列表和连接测试有账户级并发/频率上限；连接测试可能计费，429 会遵从 Retry-After。预算设置还可选新任务默认的首轮检索范围。
2. 保存并执行「测试工具调用」。这会发出一次真实模型请求，可能计费；不测试科研能力。拉取到的模型列表只说明该服务报告了哪些模型，不代表它们支持工具调用。
3. 输入目标并选择小范围或较宽的首轮检索起步方式，再确认发送材料后启动任务。起步范围不限制后续功能；agent 可根据证据缺口继续检索，报告会说明实际覆盖和未查来源。任务页显示调用数、上游 HTTP 请求、完整模型请求字符/字节和提供商用量。
...
> **托管模式还不是可交付状态。** 登录接口、按账户隔离、请求预算、按模型目的地隔离的熔断与探测上限都有离线回归；真实 TLS、第二用户和独立设备验收仍待完成。全站急停由部署者显式设置 RE0_SITE_EMERGENCY_STOP=1，不会因单个用户的模型错误自动触发。
...
| `update_plan` / `finish_report` | 更新公开行动计划、提交并校验结构化结果；报告可引用本轮论文与资源核验证据提出待人工确认的候选关联 |

未配置 Tavily 时，模型仍可搜索论文、GitHub 和 Hub，但不会得到一个假装能全网搜索的工具。全文读取仅支持开放 arXiv 与 ACL Anthology 的有界 HTML/PDF 文本；任意网页、网盘与未接入的出版社仍不支持，且不会执行陌生代码。

### 结果中的论文—资源矩阵

当同一任务取得论文元数据与 `inspect_resource` 核验结果时，结构化报告可提出论文—资源候选关联。每个关联必须指向当前任务里的论文证据和资源审计证据；矩阵把来源 ID、定位、取得时间、检查范围、各类文件状态、访问失败、许可证声明与剩余限制保留下来。候选关联不会自动认定官方归属、版本匹配或可复现性。读者可在任务结果里批量选择、预览，再确认写入自己的文献库；预览不写入，已有论文笔记不会覆盖。确认后，文献库追加保存这次关联的来源证据和确认时间；原始资源审计仍保持其观察状态。JSON、Markdown、CSV 和入库预览都来自同一矩阵数据。未关联的检查单独列出，不把失败或未关联解释成没有资源。
...
| `RE0_GUEST_ACCESS` | 默认关闭；仅在明确设置为 `true` 时，托管服务开放最多 20 个、每个 2 小时的隔离访客会话。每位访客必须自带模型 Key；访客不进入管理员账户列表 |
...
`local` 模式没有登录，也没有多用户隔离（只有一个 owner：`local`），**不要直接暴露到公网或不可信局域网**。托管模式有账户、会话、按账户的数据与密钥隔离、请求/任务配额、目的地熔断和探测限额，以及 `/login` 登录页，但**它还不是可交付状态**——真实 TLS、第二用户、独立设备和保留策略仍待验收。两种模式的边界见 [SECURITY.md](SECURITY.md)。

**登录页（`/login`）说清楚的事**：本地模式**不摆表单**，只说明这台服务只有一个所有者、没有登录这一步；托管模式先问 `/api/auth/session` 再决定显示什么，所以页面不会在不知道模式时索要凭据。若部署者显式开启 `RE0_GUEST_ACCESS`，可直接创建独立的两小时访客空间；关闭访客入口时仍由部署者 `python -m re0 auth create-user` 创建正式账户。口令**不写进任何浏览器存储**（提交后立即清空输入框），会话票是 HttpOnly Cookie，页面脚本读不到；`?next=` 只接受本站路径，`//evil.example` 这种协议相对写法会被拒。失败文案照抄服务器原话：**口令错、账户不存在、已停用、正在锁定这四种情况在服务器上是一句话、一个耗时**，页面不猜是哪一种。访客可单独删除本次数据或退出，都会撤销会话并清理其数据库记录与工作区文件。

配额分为账户、会话、全站 HTTP 和模型探测几层：RE0_REQUESTS_PER_MINUTE、RE0_TASKS_PER_HOUR、RE0_SITE_REQUESTS_PER_MINUTE 控制请求与任务；会话累计上限控制模型/工具调用；每任务默认最多 60 次上游 HTTP 请求，并保留一次模型请求用于结构化报告。本地模式未设置账户/全站 HTTP 和任务额度时不限，显式设置后两种模式都生效。
模型列表/连接探测有独立账本：每账户模型列表 6 次/分钟、连接测试 3 次/5 分钟、同一账户一次在途探测、全站最多 4 个。HTTP 429 遵从 Retry-After，只冷却当前账户与模型目的地。408、网络错误和 5xx 按不含 Key 的规范化 Base URL 分别计入目的地熔断；401/402/403/404 不会熔断其他目的地。连续 8 次目的地失败后只拒绝该服务的新任务，半开时只允许一次探测；状态随数据库重启恢复。RE0_SITE_EMERGENCY_STOP=1 是部署者控制的全站急停。拒绝说明作用范围、等待时间与是否已发出外部请求。
